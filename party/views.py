...
from party.forms import BasicPartyForm,EditForm,CancelPartyForm
...
def create():
  ...
  if request.method == "POST" and form.validate():
    ...
    if not error:
      ...
      party.save()
      return redirect(url_for('party_page.public',id=party.id))
...
def manage(page=1):
  user = User.objects.filter(email=session.get('email')).first()
  if user:
    parties = Party.objects.filter(host=user.id).order_by('-start_time').paginate(page=page,per_page=4)
...
def explore(page=1):
  ...
  display_party = Party.objects(cancel=False).order_by('-start_time').paginate(page=page,per_page=4)
  ...
  try:
    ...
    return render_template("party/explore.html",parties=parties,display_party=display_party,place=place,lng=lng,lat=lat)
  except:
    
    
    return render_template('party/explore.html',place=place,display_party=display_party)
